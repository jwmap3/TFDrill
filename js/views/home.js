/* ============================================================
   View: Home — "Get Training" hero grid + dashboard below
   ============================================================ */

window.Views = window.Views || {};

const HOME_BUTTONS = [
  { icon: "🎲", label: "Random Drill", accent: "#0d9488", onclick: "App.navigate('random')" },
  { icon: "🚒", label: "Drills", accent: "#e11d2e", onclick: "Views.openDrillsPicker()" },
  { icon: "⚔️", label: "Challenge a Crew", accent: "#2f6fed", onclick: "App.navigate('challenges')" },
  { icon: "🏆", label: "The Challenge Board", accent: "#ca8a04", onclick: "App.navigate('board')" },
  { icon: "📹", label: "Training Videos", accent: "#7c3aed", onclick: "App.navigate('videos')" },
  { icon: "⭐", label: "Favorites", accent: "#db2777", onclick: "App.navigate('favorites')" }
];
  ...
  return `
    <section class="hero-grid-section">
      ...
      <div class="button-grid">
        ${HOME_BUTTONS.map(b => `
          <button class="grid-btn" style="--accent:${b.accent}" onclick="${b.onclick}">
            <span class="grid-btn-icon-wrap">${b.icon}</span>
    ...
    <section class="dashboard">
      <h2 class="section-title">Welcome back${profile && profile.name ? ", " + App.esc(profile.name) : ""}</h2>
